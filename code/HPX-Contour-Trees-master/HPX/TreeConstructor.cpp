...
void TreeConstructor::stitch(std::vector<Value> danglings){
    ...
    //trunkStart->augmentation.inherit(trunkStart->inheritedAugmentations);
...
#else
//                for (Value c : arc->augmentation.vertices){
//                    swept[c.vertex] = INVALID_VERTEX;
  ...
  //              arc->augmentation.clear();
        ...
        [&](std::uint64_t i)
    {
        ...
        if (swept[v] == INVALID_VERTEX){
            if (this->dataManager->getValue(v) < danglings[0]){
                ...
                //arc->augmentation.sweep(this->dataManager->getValue(v));
            } else {
                int64_t min = 0;
    ...
    for (Arc* arc : this->arcMap.local){
        if (arc != nullptr){
            for (auto iter = arc->augmentation.vertices.begin(); (iter != arc->augmentation.vertices.end()); iter.operator ++()){
            LogDebug().tag(std::to_string(this->index)) << "Augmentation of "
                                                        << ( (*iter)->key.vertex  >> BLOCK_INDEX_SHIFT) << "::" << ( (*iter)->key.vertex  & VERTEX_INDEX_MASK) << " is "
                                                        << ( arc->extremum  >> BLOCK_INDEX_SHIFT) << "::" << ( arc->extremum  & VERTEX_INDEX_MASK);
            }
        }
    }
...
void TreeConstructor::countSweepsLocal(int64_t diff){
    ...
    if (currentcount % 100000 == 0 || (currentcount < 1000 && currentcount > -1000) || (currentcount < -19700000)){
        LogInfo().tag(std::to_string(this->index))<< "Current Count: " << currentcount;
    }
...
void TreeConstructor::countSweeps(std::int64_t diff){
    ...
    if (currentcount % 100000 == 0 || (currentcount < 1000 && currentcount > -1000)){
        LogInfo().tag(std::to_string(this->index))<< "Current CountSweeps: " << currentcount;
    }
...
std::vector<RemoteAnswer> TreeConstructor::continueSweepPeer(uint64_t v, const std::vector<std::uint64_t> children)
{
    ...
    //arc->augmentation.inherit(arc->inheritedAugmentations);
...
//#endif
    ...
    //Augmentation bob = finishedArc->augmentation.heritage(s);
    //saddleArc->inheritedAugmentations.push_back(bob);
#ifndef FLATAUGMENTATION
    for (auto iter = finishedArc->augmentation.vertices.begin(); (iter != finishedArc->augmentation.vertices.end()); iter.operator ++()){
        swept[(*iter)->key.vertex] = finishedArc->extremum;
    }
#else
    //for (Value c : finishedArc->augmentation.vertices){

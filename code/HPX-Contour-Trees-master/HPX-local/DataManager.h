...
public:
    ...
    void readFromFile(const std::string& fileName)
    {
        ...
        if (extension == ".vtu") {
            ...
            for (int i = reader->GetOutput()->GetPointData()->GetNumberOfArrays(); i > 0; i--){
                reader->GetOutput()->GetPointData()->RemoveArray(i);
            }
            ...
            max_value = tmp[1];
            conversionFactor = (std::numeric_limits<std::uint32_t>().max() / (max_value - min_value));
            float* temp = static_cast<float*>(grid->GetScalarPointer());
            values = (double*)malloc(grid->GetNumberOfPoints()*sizeof(double));
    ...
    void printTimes()
    {
        std::cout << "Neighbor Time: " << neighborTime << std::endl;
        std::cout << "Value Time: " << valueTime << std::endl;
    }
...
private:
    ...
    mutable hpx::util::high_resolution_timer valueTimer;
    mutable hpx::util::high_resolution_timer neighborTimer;

    mutable double valueTime = 0;
    mutable double neighborTime = 0;
